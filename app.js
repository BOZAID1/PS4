(function () {
  ...
  function go(button, event) {
    ...
    // Allow offline navigation; the destination manages its own cached page.
    return true;
  }
  for (var i = 0; i < 3; i++) {
    (function (index) {
  ...
  if ('serviceWorker' in navigator && location.protocol === 'https:') {
    var registration = null, checking = false, lastCheck = 0, reloading = false;
    function checkUpdate() {
      if (!registration || checking || navigator.onLine === false || document.hidden) { return; }
      if (new Date().getTime() - lastCheck < 15000) { return; }
      lastCheck = new Date().getTime(); checking = true;
      registration.update().then(function () {
        checking = false;
        if (registration.active && !registration.installing) { registration.active.postMessage({ type: 'CHECK_BOZAID_CACHE' }); }
      }).catch(function () { checking = false; fail(); });
    }
    function observe(worker) {
      if (!worker) { return; }
      message('جارٍ تنزيل التحديث وحفظه…');
      worker.addEventListener('statechange', function () {
        if (worker.state === 'redundant') { fail(); }
      });
    }
    message('جارٍ التحقق من التحديثات…');
    navigator.serviceWorker.addEventListener('message', function (event) {
      if (event.data && event.data.type === 'BOZAID_UPDATE_READY' && !reloading) {
        reloading = true; location.reload(); return;
      }
      if (event.data && event.data.type === 'BOZAID_CACHE_READY') { ready(); }
      if (event.data && event.data.type === 'BOZAID_CACHE_MISSING') { saved = false; fail(); }
    });
    navigator.serviceWorker.register('sw.js', { updateViaCache: 'none' }).then(function (result) {
      registration = result;
    ...
    navigator.serviceWorker.ready.then(function (result) {
      if (result.active) { result.active.postMessage({ type: 'CHECK_BOZAID_CACHE' }); }
    });
    ...
    var cache = window.applicationCache, lastLegacyCheck = 0;
    function applyLegacyUpdate() {
      try { cache.swapCache(); } catch (error) { /* Already swapped; reload the completed version. */ }
      message('اكتمل التحديث · جارٍ فتح النسخة الجديدة…', 'ready');
      location.reload();
    }
    function checkLegacyUpdate() {
      if (navigator.onLine === false || document.hidden || cache.status !== 1) { return; }
      if (new Date().getTime() - lastLegacyCheck < 15000) { return; }
      lastLegacyCheck = new Date().getTime();
      try { cache.update(); } catch (error) { /* Browser may already be checking. */ }
    }
    message('جارٍ التحقق من التحديثات…');
    cache.addEventListener('downloading', function () { message('جارٍ تنزيل التحديث…'); });
    cache.addEventListener('progress', function (event) {
      if (event.total) { message('جارٍ الحفظ… ' + Math.round(event.loaded / event.total * 100) + '٪'); }
    });
    cache.addEventListener('cached', ready);
    cache.addEventListener('noupdate', ready);
    cache.addEventListener('updateready', applyLegacyUpdate);
    cache.addEventListener('error', fail);
    cache.addEventListener('obsolete', function () { saved = false; fail(); });
    if (cache.status === 4) { applyLegacyUpdate(); }
    else if (cache.status === 1) { ready(); checkLegacyUpdate(); }
    setInterval(checkLegacyUpdate, 60000);
    window.addEventListener('online', checkLegacyUpdate);
    document.addEventListener('visibilitychange', function () { if (!document.hidden) { checkLegacyUpdate(); } });
  } else {
